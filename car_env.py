...
class CarEnv(object):
    ...
    def step(self, action):
        if self.is_discrete_action:
            ...
            action = np.clip(action, self.action_bound[0],self.action_bound[1])[0]  #*self.action_bound
    ...
    def reset(self):
        self.terminal = False
        self.car_info[:3] = np.array([self.start_point[0],self.start_point[1], -np.pi/2])
        self._update_sensor()
        return self._get_state()

    def render(self):
        if self.viewer is None:
            self.viewer = Viewer(self.viewer_xy[0],self.viewer_xy[1],self.car_info, self.sensor_info, self.obstacle_coords)
        self.viewer.render()
    ...
    def sample_action(self):
        if self.is_discrete_action:
            ...
            a = np.random.uniform(self.action_bound[0],self.action_bound[1], size=self.action_dim)
        return a
    ...
    def _update_sensor(self):
        ...
        for si in range(len(self.sensor_info)):
            ...
            win_coord = np.array([
                [0, 0],
                [self.viewer_xy[0], 0],
                [self.viewer_xy[0],self.viewer_xy[1]],
                [0, self.viewer_xy[1]],
                [0, 0],
            ])
            for oi in range(4):
...
class Viewer(pyglet.window.Window):
    ...
    def _update(self):
        ...
        for i, sensor in enumerate(self.sensors):
            sensor.vertices = [cx, cy, self.sensor_info[i, -2],self.sensor_info[i, -1]]
